...
if [ -z "$MYSQL_IP" ];        then  echo "Need to set MYSQL_IP environment";        exit 1; fi
if [ -z "$MYSQL_USER" ];      then  echo "Need to set MYSQL_USER environment";      exit 1; fi
if [ -z "$MYSQL_PASS" ];      then  echo "Need to set MYSQL_PASS environment";      exit 1; fi
if [ -z "$ADMIN_USERNAME" ];  then  echo "Need to set ADMIN_USERNAME environment";  exit 1; fi
if [ -z "$ADMIN_PASSWORD" ];  then  echo "Need to set ADMIN_PASSWORD environment";  exit 1; fi
if [ -z "$ADMIN_EMAIL" ];     then echo "Need to set ADMIN_EMAIL environment";      exit 1; fi
